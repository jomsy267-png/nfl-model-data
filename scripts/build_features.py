...
def build_team_history(sched: pl.DataFrame) -> pl.DataFrame:
    ...
    def add_rolls(g: pl.DataFrame) -> pl.DataFrame:
        g = g.with_columns([
            # Shifted series to exclude current game from rolling
        ...
        # Rolling means over last 3 and 5 lagged games
        g = g.with_columns([
...
def main():
    ...
    # Bring back game-level columns that are useful as "market priors"
    game_cols = df.select([
        "game_id","season","week","gameday","home_team","away_team","home_score","away_score",
        "spread_line","total_line"
    ])

    # Join team-centric rolls back for both home and away
    ...
    # Merge into a single game-level (home perspective) table
    feats = (
        game_cols
